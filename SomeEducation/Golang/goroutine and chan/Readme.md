## Func Pipe
В функцию передается массив функций, которые выполняют работу и передают результат через канал в следующую функцию. Каждая функция запускается в отдельной гоурутине и ждет пока закончится поток данных.  
